...
export async function generateHistoricoAction(formData: FormData): Promise<{ base64?: string, error?: string, templateName?: string }> {
    try {
        ...
        for (let i = 1; i <= 5; i++) {
            ...
            // Define RES_EDUCARMAIS e garante que HMAIS fique vazio (riscado) se não houver Educar Mais
            const isBlankYear = !formData.has(`ANO_${i}`) || formData.get(`ANO_${i}`)?.toString().trim() === "";
            const isEducarMais = formData.get(`EDUCARMAIS_${i}`) === "on";

            if (isEducarMais && !isBlankYear) {
                data[`RES_EDUCARMAIS_${i}`] = "||CLEAR_DIAG||Resolução SE nº 21/2017 – Programa Educar Mais";
                let hmaisVal = formData.get(`HMAIS_${i}`)?.toString().trim() || " ";
                data[`HMAIS_${i}`] = "||CLEAR_DIAG||" + hmaisVal;
            } else {
        ...
        if (xmlFile) {
            ...
            // 0) Limpar bordas diagonais de Educar Mais nas células que foram preenchidas (remove ||CLEAR_DIAG|| e tl2br)
            xml = xml.replace(
                /<w:tc(?:(?:(?!<\/w:tc>)[\s\S])*?)\|\|CLEAR_DIAG\|\|(?:(?:(?!<\/w:tc>)[\s\S])*?)<\/w:tc>/g,
                (match) => {
                    let m = match.replace(/\|\|CLEAR_DIAG\|\|/g, "");
                    // Limpar a propriedade de borda diagonal tl2br se existir
                    m = m.replace(/<w:tl2br[^>]*>/g, "");
                    return m;
                }
            );
